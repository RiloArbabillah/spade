...
- 60 test (`tests/`) lulus saat PR itu dibuat, memakai fixture HTTP server lokal
  ...
  23 modul. (Suite saat ini 424 test / 32 modul; lihat
...
- Impersonation hanya menyamarkan fingerprint klien. Delay/jitter sudah tersedia
  lewat `--delay`/`--max-rps`/`--jitter` dan rotasi IP lewat
  `--proxy`/`--proxy-file` (lihat [scan-engine.md](scan-engine.md)), tapi pola
  request manusiawi belum ada — lihat

...
- 60 test (`tests/`) lulus, memakai fixture HTTP server lokal — tanpa koneksi
  internet. Mencakup header impersonation, retry status + `Retry-After`,
  timeout default, cookie, validasi `--impersonate`, serta regresi 23 modul.
